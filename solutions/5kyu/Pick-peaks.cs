...
Pick peaks (https://www.codewars.com/kata/5279f6fe5ab7f447890006a7/train/csharp)

In this kata, you will write a function that returns the positions and the values of the "peaks" (or local maxima) of a numeric array.

For example, the array arr = [0, 1, 2, 5, 1, 0] has a peak at position 3 with a value of 5 (since arr[3] equals 5).

The output will be returned as a Dictionary<string, List<int>> with two key-value pairs: "pos" and "peaks". If there is no peak in the given array, 
...
public class PickPeaks
{
    public static Dictionary<string, List<int>> GetPeaks(int[] arr)
    {
      ...
      for (int i = 1; i < arr.Length; i++) {
        // Check if a peak has occured
        if (arr[i] < arr[i-1] && isRising) {
          isRising = false;
          peaks["pos"].Add(peakIndex);
          peaks["peaks"].Add(arr[i-1]);
        } 
        // Update new peak
