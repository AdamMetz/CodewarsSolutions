...
Best travel (https://www.codewars.com/kata/55e7280b40e1c4a06d0000aa/train/csharp)
...
public static class SumOfK 
{
    public static int? chooseBestSum(int t, int k, List<int> ls) {
      int bestSum = 0;
      int listSize = ls.Count;
      
      if (listSize < k) return null;
      
      // Indices for current combination sum
      ...
      while (indices[0] <= listSize - k) {
        int combinationSum = getCombinationSum(ls, indices);
        if (combinationSum <= t && combinationSum > bestSum)
          bestSum = combinationSum;
        
        // Find combination index to update
        int indexToUpdate = k - 1;
        while (indexToUpdate >= 0) {
          if (indices[indexToUpdate] == listSize - 1 - (k - 1 - indexToUpdate)) {
            indexToUpdate--;
          } else break;
        }
        ...
        // Update all the following indices
        for (int j = indexToUpdate + 1; j < k; j++)
    ...
    public static int getCombinationSum(List<int> ls, int[] indices) {
      int sum = 0;
